/*
  Update get_admin_analytics to net succeeded order_refunds against
  total_revenue_cents and total_deposits_collected_cents.

  Allocation rule (deposit-first):
    - refunds reduce deposit totals first
    - remainder reduces revenue (subtotal+tax)
*/
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'total_revenue_cents', COALESCE((
      SELECT SUM(o.subtotal_cents + o.tax_cents)
      FROM orders o
    ...
    -- subtract refunds that exceed deposit amount (deposit-first: excess reduces revenue)
    - COALESCE((
      SELECT SUM(GREATEST(0, order_refund_total - order_deposit))
      FROM (
        SELECT
          o.id,
    ...
    -- subtract deposit-portion of refunds (deposit is reduced first)
    - COALESCE((
