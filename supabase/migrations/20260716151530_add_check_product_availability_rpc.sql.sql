-- ============================================================
-- Tables & Chairs Phase 1: check_product_availability RPC
-- ============================================================
...
BEGIN
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM inventory_products ip WHERE ip.id = r.product_id
  );
  ...
  RETURN QUERY
  WITH
  -- Aggregate requested items (sum duplicates)
  requested AS (
    SELECT
  ...
  -- Individual product order items (pricing_context = 'standalone')
  direct_booked AS (
    SELECT
      oi.product_id,
  ...
  bundle_booked AS (
    SELECT
      (comp->>'product_id')::uuid AS product_id,
  ...
  total_booked AS (
    SELECT product_id, SUM(qty_booked) AS total_reserved
    FROM (
      SELECT product_id, qty_booked FROM direct_booked
      UNION ALL
      SELECT product_id, qty_booked FROM bundle_booked
    ) combined
    GROUP BY product_id
  )
  SELECT
    ...
    (ip.total_quantity - ip.temp_unavailable_qty - COALESCE(tb.total_reserved, 0) - COALESCE(r.quantity, 0))::integer,
    (COALESCE(r.quantity, 0) <= ip.total_quantity - ip.temp_unavailable_qty - COALESCE(tb.total_reserved, 0))
  FROM inventory_products ip
  LEFT JOIN total_booked tb ON tb.product_id = ip.id
  LEFT JOIN requested r ON r.product_id = ip.id
  WHERE r.product_id IS NOT NULL;
END;
$function$;

-- Revoke default PUBLIC execution; grant to anon, authenticated, service_role
