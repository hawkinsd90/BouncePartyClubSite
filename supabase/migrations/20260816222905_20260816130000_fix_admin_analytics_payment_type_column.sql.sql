/*
# Fix get_admin_analytics: remove non-existent payment_type column reference

## Problem
Migration 20260702231624 introduced references to `p.payment_type` in two
places within get_admin_analytics(), but the `payments` table has no
`payment_type` column — only `type`. This causes the entire analytics RPC
to fail with "column p.payment_type does not exist" whenever the admin
analytics page loads.

## Changes
- Replace `(p.payment_type = 'deposit' OR p.type = 'deposit')` with
  `p.type = 'deposit'` in both the main deposit sum and the subquery.
- No other logic changes.

## Data Safety
- No table changes, no data changes. Only the function definition is replaced.
*/
