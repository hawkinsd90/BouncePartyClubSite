/*
# Phase 3A: Add caller-aware visibility enforcement to check_product_availability

1. Modified Functions
- `check_product_availability` — SECURITY DEFINER function updated to enforce
  product/category visibility for public callers while preserving full access
  for admin/master/service_role.

2. Security Changes
- Added v_is_privileged boolean variable
- Privilege is true only for:
  - Authenticated admin/master found through user_roles
  - auth.role() = 'service_role'
- Missing JWT claims are NOT treated as privileged
- For ordinary public callers, requested products must satisfy:
  - product active = true
  - product public_visible = true
  - category_id is not null
  - category active = true
  - category public_visible = true
- Same rule applied to both requested-product validation and final returned rows
- Privileged callers may check hidden, inactive, and uncategorized products

3. Preserved Properties
- Parameter names, types, and default value unchanged
- Return columns unchanged
- LANGUAGE plpgsql
- SECURITY DEFINER
- SET search_path TO 'public'
- Owner preserved via CREATE OR REPLACE
- Existing grants preserved automatically
*/
...
BEGIN
  ...
  -- Validate all requested products exist (with visibility check for public callers)
