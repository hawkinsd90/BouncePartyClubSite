/*
# Phase 3B Stage 1 Correction: save_product_bundle

## Bug 1: Raw numeric cast leaks PostgreSQL errors

The component quantity validation at line 108 did:

    v_component_qty := (v_component ->> 'quantity_per_bundle')::numeric;

If the JSON value is text like "abc", an empty string, or other non-numeric
input, PostgreSQL raises an uncontrolled "invalid input syntax for type
numeric" error before any validation can run.

Fix: read the raw text, validate it is a positive whole integer using
regex and explicit checks, and only cast to numeric after it passes.

## Bug 2: Missing-product query aggregates NULL ip.id

The missing-product detection at line 132 did:

    SELECT array_agg(ip.id::text) ...
    FROM unnest(v_product_ids) AS ip_id
    LEFT JOIN inventory_products ip ON ip.id = ip_id
    WHERE ip.id IS NULL;

Since ip.id IS NULL for missing rows, array_agg(ip.id::text) returns NULL,
producing an empty product list in the error message.

Fix: aggregate the unnest value (ip_id) instead of ip.id.
*/
