/*
  # Fix get_admin_analytics: fix avg_lead_time_days EXTRACT expression

  Subtracting two dates in Postgres returns an integer (number of days), not an
  interval. EXTRACT(EPOCH FROM integer) is invalid. Cast the difference to an
  interval before extracting the epoch, or simply cast directly to numeric.
*/
