/*
  # Fix draft → confirmed status transition for invoice payment flow

  ## Problem
  The order status transition trigger only allowed draft → pending_review | cancelled | void.
  When a customer pays via the invoice link, the order is in "draft" status and charge-deposit
  tries to update it directly to "confirmed". This caused the trigger to raise an exception,
  resulting in "Payment was processed but order update failed."

  ## Fix
  Add "confirmed" as a valid transition from "draft" so invoice payments can succeed.
*/
