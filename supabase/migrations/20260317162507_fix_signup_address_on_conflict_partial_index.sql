/*
  # Fix create_signup_address ON CONFLICT crash

  ## Root Cause
  The trigger used `ON CONFLICT (address_key)` but `addresses_address_key_unique`
  is a PARTIAL unique index (`WHERE address_key IS NOT NULL`), not a constraint.
  PostgreSQL requires the ON CONFLICT clause to exactly match the partial index
  predicate. Without `WHERE address_key IS NOT NULL`, Postgres cannot resolve
  which index to use and raises:
    "there is no unique or exclusion constraint matching the ON CONFLICT specification"

  ## Fix
  - Change `ON CONFLICT (address_key)` to
    `ON CONFLICT (address_key) WHERE address_key IS NOT NULL`
    in the create_signup_address step.
  - Also fixes first_name/last_name: the trigger was crashing before reaching
    the customer INSERT that sets names, so existing customers got email-prefix
    names. The trigger now completes fully for new signups.
  - No schema changes — the index already exists and is correct.
*/
...
BEGIN
  ...
  BEGIN
    ...
    -- Create address from signup metadata.
    -- ON CONFLICT must match the partial index predicate: WHERE address_key IS NOT NULL
