/*
  Recreate get_admin_analytics to exclude orders placed by admin/master accounts.
  Admins are identified via customers.user_id -> user_roles.role IN ('admin','master').
  Orders with no customer link (customer_id IS NULL) are still included — those are
  anonymous bookings, not admin test orders.
*/
...
BEGIN
  -- Helper: inline subquery that returns TRUE for orders belonging to admin/master accounts
  -- Used as NOT EXISTS(...) to exclude them from every metric.
  -- Pattern: NOT EXISTS (SELECT 1 FROM customers c JOIN user_roles ur ON ur.user_id = c.user_id
