/*
# Stage B — v1 compatibility + product RPC test harness (TO BE DROPPED)
Verifies:
- v1 save_product_bundle does NOT erase Stage B config
- v1 save_inventory_product does NOT erase threshold
- v2 save_inventory_product_v2 saves threshold
- Negative product threshold rejected
- Failed update preserves previous state
*/
...
BEGIN
  ...
  PERFORM public.save_product_bundle_v2(
    ...
    15000, 'selected', ARRAY[v_tables_cat], ARRAY[v_block_party_id], '[]'::jsonb
  );

  -- CASE 18: v1 save_product_bundle (Stage A PackageForm style) does NOT erase threshold/mode/eligibility
  ...
  PERFORM public.save_product_bundle(
    'update', v_test_bundle_id, 'stage-b-test-prod-only', 'STAGE-B-TEST Product Only',
    null, null, null, null, false, false, true, true, true, false, 999,
    jsonb_build_array(jsonb_build_object('product_id', v_table_prod, 'quantity_per_bundle', '2'))
  );
  SELECT addon_qualifying_threshold_cents, inflatable_eligibility_mode INTO v_threshold, v_mode
  FROM product_bundles WHERE id = v_test_bundle_id;
  ...
  BEGIN
    PERFORM public.save_product_bundle(
      'update', 'cde247f7-522c-44f8-a6fc-a0bea426acb4'::uuid, 'celebration-seating',
      'Celebration Seating', null, null, 10000, 5000, true, false, true, true, false, false, 10,
  ...
  BEGIN
    PERFORM public.save_inventory_product_v2(
      'create', v_test_product_id, 'stage-b-test-product', 'STAGE-B-TEST Product',
      null, 10, 0, true, true, v_tables_cat, 500, 1000, 200, true, false, 15000
    );
  ...
  PERFORM public.save_inventory_product(
    'update', v_test_product_id, 'stage-b-test-product', 'STAGE-B-TEST Product',
    null, 10, 0, true, true, v_tables_cat, 500, 1000, 200, true, false
  );
  SELECT addon_qualifying_threshold_cents INTO v_threshold FROM product_pricing WHERE product_id = v_test_product_id;
  IF v_threshold = 15000 THEN
  ...
  PERFORM public.save_inventory_product_v2(
    'update', v_test_product_id, 'stage-b-test-product', 'STAGE-B-TEST Product',
    null, 10, 0, true, true, v_tables_cat, 500, 1000, 200, true, false, 20000
  );
  SELECT addon_qualifying_threshold_cents INTO v_threshold FROM product_pricing WHERE product_id = v_test_product_id;
  ...
  BEGIN
    PERFORM public.save_inventory_product_v2(
      'update', v_test_product_id, 'stage-b-test-product', 'STAGE-B-TEST Product',
      null, 10, 0, true, true, v_tables_cat, 500, 1000, 200, true, false, -5
    );
  ...
  BEGIN
    PERFORM public.save_inventory_product(
      'update', 'aeed01e4-73a3-40a0-bb01-82ec2f60ee73', 'six-foot-rectangular-table',
      'Six-foot Rectangular Table', null, 50, 0, true, true, v_tables_cat, 100, 1000, 200, true, false
    );
  ...
  -- (test bundle 666... has carnival water component; attempt failing update, verify component remains)
  BEGIN
  ...
  -- (test bundle 111... has block_party eligibility; attempt failing update, verify eligibility remains)
  BEGIN
    PERFORM public.save_product_bundle_v2(
      'update', v_test_bundle_id, 'stage-b-test-prod-only', 'STAGE-B-TEST Product Only',
