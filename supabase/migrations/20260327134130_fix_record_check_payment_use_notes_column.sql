/*
  # Fix record_check_payment — write check number into changelog notes column

  ## Changes vs previous version (131256)
  1. The `balance_due_cents` changelog INSERT now includes the `notes` column.
     Check number is written as 'Check number: <n>' when provided, NULL otherwise.
  2. The now-redundant `v_notes_text` variable and its build block are removed.
  3. The separate tip-note string that had no destination is removed.
  4. Every other block is byte-for-byte identical to the cash RPC baseline
     (040745_fix_record_cash_payment_rpc_payment_columns).

  ## Changelog schema confirmed (information_schema):
    id, order_id, user_id, field_changed, old_value, new_value,
    change_type, created_at, notes
*/
...
BEGIN
  ...
  INSERT INTO order_changelog (
    ...
    'check_payment',
    CASE
      WHEN p_check_number IS NOT NULL AND btrim(p_check_number) <> ''
      THEN 'Check number: ' || btrim(p_check_number)
      ELSE NULL
    END
  );
