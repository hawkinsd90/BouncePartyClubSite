/* Stage B — fix compat harness: add missing p_image_url param for v1 product RPC */
...
BEGIN
  ...
  -- CASE 15: v2 create product with threshold (16 args + threshold = 17)
  BEGIN
    PERFORM public.save_inventory_product_v2(
      ...
      null::text, null::text, 10, 0, true, true, v_tables_cat, 500, 1000, 200, true, false, 15000
    );
  ...
  -- CASE 12: v1 product update (16 args, no threshold) does NOT erase stored threshold
  PERFORM public.save_inventory_product(
    'update'::text, v_test_product_id, 'stage-b-test-product'::text, 'STAGE-B-TEST Product'::text,
    null::text, null::text, 10, 0, true, true, v_tables_cat, 500, 1000, 200, true, false
  );
  SELECT addon_qualifying_threshold_cents INTO v_threshold FROM product_pricing WHERE product_id = v_test_product_id;
  IF v_threshold = 15000 THEN
  ...
  PERFORM public.save_inventory_product_v2(
    'update'::text, v_test_product_id, 'stage-b-test-product'::text, 'STAGE-B-TEST Product'::text,
    null::text, null::text, 10, 0, true, true, v_tables_cat, 500, 1000, 200, true, false, 20000
  );
  SELECT addon_qualifying_threshold_cents INTO v_threshold FROM product_pricing WHERE product_id = v_test_product_id;
  ...
  BEGIN
    PERFORM public.save_inventory_product_v2(
      'update'::text, v_test_product_id, 'stage-b-test-product'::text, 'STAGE-B-TEST Product'::text,
      null::text, null::text, 10, 0, true, true, v_tables_cat, 500, 1000, 200, true, false, -5
    );
  ...
  BEGIN
    PERFORM public.save_inventory_product(
      ...
      'Six-foot Rectangular Table'::text, null::text, null::text, 50, 0, true, true, v_tables_cat, 100, 1000, 200, true, false
    );
