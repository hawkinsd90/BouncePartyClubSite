...
export function PendingOrderCard({ order, onUpdate }: { order: any; onUpdate: () => void }) {
  ...
  async function loadContact() {
    const { data } = await supabase
      ...
      .eq('email', order.customers?.email)
      .maybeSingle();
                        ...
                        <td style="padding: 30px;">
                          ...
                          <div style="background-color: #f8fafc; border-radius: 6px; padding: 20px; margin: 25px 0;">
                            ...
                            <table width="100%" cellpadding="6" cellspacing="0">
                              ...
                              <tr>
                                <td style="color: #64748b; font-size: 14px;">Travel Fee${(orderWithItems.travel_total_miles ?? 0) > 0 ? ` (${(orderWithItems.travel_total_miles ?? 0).toFixed(1)} mi)` : ''}:</td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-8 max-w-md w-full">
            ...
            <div className="bg-slate-50 rounded-lg p-4 mb-6">
              <p className="text-sm text-slate-600 mb-2"><strong>Customer:</strong> {getCustomerDisplayName()}</p>
