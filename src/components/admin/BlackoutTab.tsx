...
import { Calendar, UserX, MapPin, Plus, Trash2, Edit2, Save, X } from 'lucide-react';
...
export function BlackoutTab() {
  const [activeTab, setActiveTab] = useState<'dates' | 'contacts' | 'addresses'>('dates');
  ...
  async function fetchData() {
    setLoading(true);
    try {
      const [datesRes, contactsRes, addressesRes] = await Promise.all([
