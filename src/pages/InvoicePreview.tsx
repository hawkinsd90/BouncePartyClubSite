...
export function InvoicePreview() {
  ...
  const handleDownloadPDF = async () => {
    // On iOS/mobile, trigger print which will show save to files/share options
    // This is more straightforward than the desktop print dialog
    document.body.classList.add('print-invoice-preview');
    ...
    if (isIOS) {
      // On iOS, window.print() opens a share sheet with PDF option
      window.print();
      setTimeout(() => document.body.classList.remove('print-invoice-preview'), 250);
    } else {
      // On desktop, just use regular print
      window.print();
      setTimeout(() => document.body.classList.remove('print-invoice-preview'), 250);
    }
  };
    ...
    <div className="invoice-preview-route min-h-screen bg-slate-50 py-6 px-4">
      ...
      <div className="max-w-3xl mx-auto mb-4 no-print">
        ...
        <div className="block sm:hidden space-y-3">
          <button
            onClick={handleBack}
            className="w-full inline-flex items-center justify-center bg-white hover:bg-slate-50 text-slate-700 font-semibold py-3 px-4 rounded-lg border border-slate-200 transition-colors"
          >
          ...
          <button
            ...
            className="w-full flex items-center justify-center bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg transition-colors shadow-lg"
          >
          ...
          <button
            onClick={handlePrint}
            className="w-full flex items-center justify-center bg-slate-600 hover:bg-slate-700 text-white font-semibold py-3 px-4 rounded-lg transition-colors"
          >
          ...
          <div className="text-center">
            <p className="text-xs text-slate-600">
              Tap "Download PDF" to save or share this invoice
            </p>
          </div>
        </div>
